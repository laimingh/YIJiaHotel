<template>
    <div id="orderDetail">
          ...
          <mu-col>
            <mu-card class="card" raised>
              ...
              <mu-card-actions class="actions">
                ...
                <mu-button flat @click="deleteOrder()" color="error">删除</mu-button>
                <mu-button color="error" @click="opentuifang = true" v-show="order.orderStatus >= 0 && order.orderStatus <2">退房</mu-button>
      ...
      <mu-dialog title="退房" width="360" :open.sync="opentuifang" :loading="canceltuifang">
        确认退房吗？
        <mu-button slot="actions" flat color="secondary" @click="opentuifang = false">返回</mu-button>
        <mu-button slot="actions" flat color="primary" @click="canceltuifang()">确认退房</mu-button>
      </mu-dialog>
    </div>
</template>

<script>
  import {getOrderById, cancelOrder, canceltuifang,payOrder, deleteOrderById} from "@/api/order";
  ...
  export default {
    ...
    methods:{
      ...
      canceltuifang(){
        this.canceltuifang = true
        canceltuifang(this.order.orderId).then(res => {
          console.log(res.console);

          if (res.code === 1000){
            this.$toast.success("退房成功！")
            this.$router.go(0)
          } else {
            this.$toast.info("退房失败，请稍后再试！")
          }
          this.cancelLoading = false
        }).catch(err => {
          this.$toast.error("退房失败，原因："+ err.toString())
          this.cancelLoading = false
        })
